...
plt.scatter(df['convolution_flops'],y,color='blue',label='Data points')
plt.title('Convolution Time vs. FLOPS')
plt.xlabel('flops')
plt.ylabel('conv_time')
plt.legend()
plt.grid(True)
plt.show()

plt.figure(figsize=(10,6))
plt.scatter(df['convolution_bytes'],y,color='blue',label='Data points')
plt.title('Convolution Time vs. Bytes')
plt.xlabel('bytes')
plt.ylabel('conv_time')
plt.legend()
plt.grid(True)
...
for image_size in validation['imagesize']:
    imagesize = image_size * image_size
    test.append([imagesize * (3 * 2), imagesize * (8)]) #2 global mem accesses * 4 bytes
